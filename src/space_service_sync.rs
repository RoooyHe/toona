...
use crate::{
    home::{
        ...
        spaces_bar::{JoinedSpaceInfo, SpacesListUpdate, enqueue_spaces_list_update},
    },
...
pub enum SpaceRequest {
    ...
    UnsubscribeFromSpaceRoomList { space_id: OwnedRoomId },
...
pub async fn space_service_loop(space_service: SpaceService, client: Client) -> anyhow::Result<()> {
    ...
    let (space_request_sender, mut receiver) =
        tokio::sync::mpsc::unbounded_channel::<SpaceRequest>();
    // Give the request sender channel endpoint to the RoomsList widget.
    enqueue_rooms_list_update(RoomsListUpdate::SpaceRequestSender(
    ...
    let get_or_spawn_space_room_list = async |space_room_list_tasks: &mut HashMap<
        OwnedRoomId,
        (UnboundedSender<SpaceRoomListRequest>, JoinHandle<()>),
    >,
                                              space_id: &OwnedRoomId,
                                              parent_chain: &ParentChain|
        ...
        match space_room_list_tasks.entry(space_id.clone()) {
            ...
            Entry::Vacant(vac) => {
                let (sender, receiver) =
                    tokio::sync::mpsc::unbounded_channel::<SpaceRoomListRequest>();
                let space_room_list = space_service.space_room_list(space_id.clone()).await;
                let join_handle = Handle::current().spawn(space_room_list_loop(
                    space_id.clone(),
                    parent_chain.clone(),
                    receiver,
                    space_room_list,
                ...
                vac.insert((sender, join_handle)).0.clone()
            }
        }
    };
    ...
    if LOG_SPACE_SERVICE_DIFFS {
        log!("space_service: initial set: {all_joined_spaces:?}");
    }
...
async fn add_new_space(space: &SpaceRoom, client: &Client) {
    let space_avatar_opt = if let Some(url) = &space.avatar_url {
        fetch_space_avatar(url.clone(), client)
            ...
            .inspect_err(|e| {
                error!(
                    "Failed to fetch avatar for new space {:?} ({}): {e}",
                    space.display_name, space.room_id
                )
            })
            .ok()
    } else {
    ...
    let space_avatar =
        space_avatar_opt.unwrap_or_else(|| utils::avatar_from_room_name(Some(&space.display_name)));
...
async fn optimize_remove_then_add_into_update(
    ...
    match peekable_diffs.peek() {
        Some(VectorDiff::Insert {
            ...
            if LOG_SPACE_SERVICE_DIFFS {
                log!(
                    "Optimizing {remove_diff:?} + Insert({insert_index}) into Update for space {}",
                    space.room_id
                );
            }
        ...
        Some(VectorDiff::PushFront { value: new_space }) if space.room_id == new_space.room_id => {
            if LOG_SPACE_SERVICE_DIFFS {
                log!(
                    "Optimizing {remove_diff:?} + PushFront into Update for space {}",
                    space.room_id
                );
            }
            update_space(&space, new_space, client).await;
        ...
        Some(VectorDiff::PushBack { value: new_space }) if space.room_id == new_space.room_id => {
            if LOG_SPACE_SERVICE_DIFFS {
                log!(
                    "Optimizing {remove_diff:?} + PushBack into Update for space {}",
                    space.room_id
                );
            }
            update_space(&space, new_space, client).await;
...
async fn update_space(old_space: &SpaceRoom, new_space: &SpaceRoom, client: &Client) {
    let new_space_id = new_space.room_id.clone();
    if old_space.room_id == new_space_id {
        // Handle state transitions for a space.
        if LOG_SPACE_SERVICE_DIFFS {
            log!(
                "Space {:?} ({new_space_id}) state went from {:?} --> {:?}",
        ...
        if old_space.state != new_space.state {
            match new_space.state {
                Some(RoomState::Banned) => {
                    ...
                    log!(
                        "Removing Banned space: {:?} ({new_space_id})",
                        new_space.display_name
                    );
                ...
                Some(RoomState::Left) => {
                    log!(
                        "Removing Left space: {:?} ({new_space_id})",
                        new_space.display_name
                    );
                ...
                Some(RoomState::Joined) => {
                    log!(
                        "update_space(): adding new Joined space: {:?} ({new_space_id})",
                        new_space.display_name
                    );
                    add_new_space(new_space, client).await;
                    return;
                ...
                Some(RoomState::Invited) => {
                    log!(
                        "update_space(): adding new Invited space: {:?} ({new_space_id})",
                        new_space.display_name
                    );
                    add_new_space(new_space, client).await;
                    return;
                ...
                None => {
                    error!(
                        "WARNING: UNTESTED: new space {} ({}) RoomState is None",
                        new_space.display_name, new_space.room_id
                    );
                }
            }
        }
        ...
        if old_space.canonical_alias != new_space.canonical_alias {
            log!(
                "Updating space {} alias: {:?} --> {:?}",
                new_space_id,
                old_space.canonical_alias,
                new_space.canonical_alias
            );
            ...
            Handle::current().spawn(async move {
                let space_avatar_opt = if let Some(url) = url_opt {
                    fetch_space_avatar(url, &client2)
                        ...
                        .inspect_err(|e| {
                            error!(
                                "Failed to fetch avatar for space {:?} ({}): {e}",
                                space_display_name, space_id
                            )
                        })
                        .ok()
                } else {
                ...
                let avatar = space_avatar_opt
                    .unwrap_or_else(|| utils::avatar_from_room_name(Some(&space_display_name)));
                enqueue_spaces_list_update(SpacesListUpdate::UpdateSpaceAvatar {
                    space_id,
                    avatar,
                });
            });
        ...
        if old_space.num_joined_members != new_space.num_joined_members {
            log!(
                "Updating space {} joined members: {} --> {}",
                new_space_id,
                old_space.num_joined_members,
                new_space.num_joined_members
            );
        ...
        if old_space.world_readable != new_space.world_readable {
            log!(
                "Updating space {} world readable: {:?} --> {:?}",
                new_space_id,
                old_space.world_readable,
                new_space.world_readable
            );
        ...
        if old_space.guest_can_join != new_space.guest_can_join {
            log!(
                "Updating space {} guest can join: {:?} --> {:?}",
                new_space_id,
                old_space.guest_can_join,
                new_space.guest_can_join
            );
        ...
        if old_space.children_count != new_space.children_count {
            log!(
                "Updating space {} children count: {:?} --> {:?}",
                new_space_id,
                old_space.children_count,
                new_space.children_count
            );
        ...
        warning!(
            "UNTESTED SCENARIO: update_space(): removing old room {}, replacing with new room {}",
            old_space.room_id,
            new_space_id,
        );
...
async fn fetch_space_avatar(
    url: OwnedMxcUri,
    client: &Client,
) -> matrix_sdk::Result<FetchedRoomAvatar> {
...
fn is_space(sr: &SpaceRoom) -> bool {
    sr.children_count > 0 || matches!(sr.room_type, Some(RoomType::Space))
}
...
async fn space_room_list_loop(
    ...
    let (mut all_rooms_in_space, mut space_room_stream) =
        space_room_list.subscribe_to_room_updates();
    handle_subspaces(
        &space_id,
        &parent_chain,
        &mut known_subspaces,
...
fn handle_subspaces<'a>(
    ...
    for sr in changed_space_rooms.filter(|&sr| is_space(sr)) {
        ...
        {
            error!(
                "BUG: failed to send subscribe request to nested/subspace {}.",
                sr.room_id
            );
        }
    }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SpaceRoomListAction::UpdatedChildren {
                ...
                .debug_struct("SpaceRoomListAction::UpdatedChildren")
            ...
            SpaceRoomListAction::PaginationState {
                ...
                .debug_struct("SpaceRoomListAction::PaginationState")
                .field("space_id", space_id)
                .field("parent_chain", &parent_chain)
                .field("state", state)
                .finish(),
            SpaceRoomListAction::PaginationError { space_id, error } => f
                .debug_struct("SpaceRoomListAction::PaginationError")
                .field("space_id", space_id)
                .field("error", error)
                .finish(),
        }
