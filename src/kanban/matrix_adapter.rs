...
impl MatrixKanbanAdapter {
    ...
    pub async fn get_all_kanban_spaces(
        &self,
    ) -> Result<Vec<crate::kanban::state::kanban_state::KanbanList>> {
        ...
        for room in all_rooms {
            ...
            if let Some(topic) = room.topic() {
                ...
                if topic.contains("[kanban-list]") {
                    ...
                    let card_ids: Vec<OwnedRoomId> =
                        card_rooms.iter().map(|r| r.room_id().to_owned()).collect();
                    ...
                    log!(
                        "Kanban list: {} ({}) with {} cards",
                        lists.last().unwrap().name,
                        room_id,
                        card_count
                    );
                }
    ...
    pub async fn create_space(&self, name: &str) -> Result<OwnedRoomId> {
        ...
        log!(
            "✅✅✅ Successfully created kanban space: {} ({})",
            name,
            space_id
        );
        Ok(space_id)
    ...
    pub async fn create_card(&self, space_id: &RoomId, title: &str) -> Result<OwnedRoomId> {
        use matrix_sdk::ruma::{
            api::client::room::create_room::v3::{Request as CreateRoomRequest, RoomPreset},
        };

        log!(
            "🎴 create_card: Creating card '{}' in space {}",
            title,
            space_id
        );
        ...
        let position = match self.client.get_room(space_id) {
            Some(space_room) => match self.get_card_list_from_state(&space_room).await {
                Ok(card_ids) => {
                ...
                Err(e) => {
                    log!(
                        "⚠️ Failed to get existing cards, using default position: {:?}",
                        e
                    );
                    1000.0
                }
        ...
        log!(
            "✓✓✓✓ Created kanban card: {} in space {} ({}) with position {}",
            title,
            space_id,
            card_room_id,
            position
    ...
    ) -> Result<crate::kanban::state::kanban_state::KanbanCard> {
    ...
    pub async fn save_card_metadata(
        &self,
        ...
        log!(
            "💾 [2/3] Saving metadata as timeline message for {} - title: {}, status: {:?}, tags: {:?}, end_time: {:?}",
            card.id,
            card.title,
            card.status,
            card.tags,
        ...
        let content =
            RoomMessageEventContent::text_plain(format!("__KANBAN_METADATA__:{}", metadata_json));
    ...
    pub async fn save_card_todos(
        ...
        log!(
            "✓ Saved todos successfully, event_id: {:?}",
            response.event_id
        );
        Ok(())
    }
                                ...
                                if let SyncRoomMessageEvent::Original(original) = room_msg {
                                    if let MessageType::Text(text) = &original.content.msgtype {
                                        ...
                                        if let Some(json_str) =
                                            body.strip_prefix("__KANBAN_METADATA__:")
                                        {
                                            ...
                                            match serde_json::from_str::<CardMetadataRaw>(json_str)
                                            {
                                                Ok(metadata) => {
                                                    log!(
                                                        "✅ Loaded card metadata: title={}, tags={:?}, end_time={:?}",
                                                        metadata.title,
                                                        metadata.tags,
                                                        metadata.end_time
                                                    );
    ...
    async fn load_card_todos(
        &self,
        room: &Room,
    ) -> Result<Vec<crate::kanban::state::kanban_state::TodoItem>> {
        ...
        match self.client.send(request).await {
            Ok(response) => {
                log!(
                    "📖 Got {} state events from server",
                    response.room_state.len()
                );
                            ...
                            if event_type_str == "m.kanban.card.todos" {
                                ...
                                if let Some(content) = json.get("content") {
                                    ...
                                    if let Some(todos_array) =
                                        content.get("todos").and_then(|v| v.as_array())
                                    {
                                        log!("📖 Todos array length: {}", todos_array.len());
                                        let todos: Vec<
                                            crate::kanban::state::kanban_state::TodoItem,
                                        > = todos_array
                                            .iter()
                                            .filter_map(|v| serde_json::from_value(v.clone()).ok())
                                            .collect();
                                        log!(
                                            "✅ Loaded {} todos successfully from server",
                                            todos.len()
                                        );
                                        return Ok(todos);
                                    }
    ...
    async fn add_card_to_space(&self, space_id: &RoomId, card_room_id: &RoomId) -> Result<()> {
        log!(
            "🔗 add_card_to_space: Adding card {} to space {}",
            card_room_id,
            space_id
        );

        let space = self.client.get_room(space_id).context("Space not found")?;
        ...
        if !space.is_space() {
            error!(
                "⚠️⚠️⚠️ WARNING: Room {} is NOT a Space (is_space=false)!",
                space_id
            );
            error!("⚠️ This is a known server bug on test.palpo.im");
            ...
            log!(
                "✓ Verified: Room {} is a proper Space (is_space=true)",
                space_id
            );
        }
        ...
        log!(
            "🔗 Step 1: Sending m.space.child event in space {} with state_key={}",
            space_id,
            card_room_id
        );
        match space
            ...
            .await
        {
            Ok(response) => {
                log!(
                    "✓ Sent m.space.child event successfully, event_id: {:?}",
                    response.event_id
                );
        ...
        log!(
            "🔗 Step 2: Sending m.space.parent event in room {} with state_key={}",
            card_room_id,
            space_id
        );
        match card_room
            ...
            .await
        {
            Ok(response) => {
                log!(
                    "✓ Sent m.space.parent event successfully, event_id: {:?}",
                    response.event_id
                );
                tokio::time::sleep(tokio::time::Duration::from_millis(300)).await;
            }
        ...
        match self.get_card_list_from_state(&space).await {
            Ok(mut card_ids) => {
                ...
                if !card_ids.contains(&card_room_id.to_owned()) {
                    ...
                    log!(
                        "🔗 Sending m.kanban.cards event with {} cards...",
                        card_ids.len()
                    );
                        ...
                        .await
                    {
                        Ok(response) => {
                            log!(
                                "✓ m.kanban.cards backup event sent, event_id: {:?}",
                                response.event_id
                            );
                    ...
                    log!(
                        "✓ Added card {} to backup list (total: {} cards)",
                        card_room_id,
                        card_ids.len()
                    );
                } else {
            ...
            Err(e) => {
                log!(
                    "⚠ Warning: Failed to get existing card list: {:?}, creating new list",
                    e
                );
                // 如果获取失败，创建新列表
                let cards_content = serde_json::json!({
        ...
        crate::kanban::local_cache::add_card_to_space_cache(
            space_id.to_owned(),
            card_room_id.to_owned(),
        );
    ...
    async fn get_card_list_from_state(&self, space: &Room) -> Result<Vec<OwnedRoomId>> {
        ...
        log!(
            "📖 get_card_list_from_state: Reading from space {}",
            space_id
        );
        ...
        match space.get_state_event(event_type, "").await {
            Ok(Some(raw_event)) => {
                ...
                if let Ok(json_str) = serde_json::to_string(&raw_event) {
                    ...
                    if let Ok(json) = serde_json::from_str::<serde_json::Value>(&json_str) {
                        ...
                        // 检查 content.card_ids
                        if let Some(content) = json.get("content") {
                            if let Some(card_ids_array) =
                                content.get("card_ids").and_then(|v| v.as_array())
                            {
                                let card_ids: Vec<OwnedRoomId> = card_ids_array
                                    ...
                                    .collect();
                                log!(
                                    "✓ Parsed {} card IDs from m.kanban.cards state",
                                    card_ids.len()
                                );
            ...
            Ok(None) => {
                log!(
                    "📖 No m.kanban.cards state event found in space {}",
                    space_id
                );
                Ok(Vec::new())
            }
    ...
    async fn get_space_children(&self, space: &Room) -> Result<Vec<Room>> {
        ...
        log!(
            "👶 get_space_children: Getting children for space: {}",
            space_id
        );
        ...
        let all_rooms = self.client.rooms();
        log!(
            "   Checking {} total rooms for m.space.child relationship...",
            all_rooms.len()
        );
        ...
        if !children_from_space_child.is_empty() {
            log!(
                "✓✓ Found {} children from m.space.child events (Matrix standard)",
                children_from_space_child.len()
            );
        ...
        match self.get_card_list_from_state(space).await {
            Ok(card_ids) if !card_ids.is_empty() => {
                log!(
                    "✓ Found {} cards from m.kanban.cards backup",
                    card_ids.len()
                );
                ...
                if !children.is_empty() {
                    log!(
                        "✓✓ Loaded {} child rooms from m.kanban.cards backup",
                        children.len()
                    );
                    return Ok(children);
                }
        ...
        let cached_card_ids =
            crate::kanban::local_cache::get_cards_from_cache(&space_id.to_owned());
        ...
        log!(
            "⚠️ No children found for space {} using any strategy",
            space_id
        );
        Ok(Vec::new())
    }
    ...
    pub async fn send_activity(
        ...
        log!(
            "💬 Sending activity to card {}: type={:?}, text={}",
            card_id,
            activity_type,
            text
        );
        ...
        if matches!(activity_type, ActivityType::Comment) {
            ...
            room.send(content).await.context("Failed to send comment")?;
        ...
        room.send_raw("m.room.message", raw_content)
            .await
    ...
    pub async fn load_activities(
        ...
        let mut request =
            matrix_sdk::ruma::api::client::message::get_message_events::v3::Request::new(
                card_id.to_owned(),
                    ...
                    AnyMessageLikeEvent::RoomMessage(room_msg) => {
                        ...
                        match &content.msgtype {
                            MessageType::Text(text_content) => {
                                ...
                                log!(
                                    "📖 Parsing message: event_id={}, sender={}, body='{}'",
                                    event_id,
                                    sender,
                                    body
                                );
                                ...
                                // 跳过卡片元数据消息
                                if body.starts_with("Card metadata:")
                                    || body.starts_with("Card Metadata:")
                                    || body.starts_with("__KANBAN_METADATA__")
                                {
                                    log!("  ⏭️ Skipping metadata message");
                                    return None;
                                }

                                // 普通评论
                                log!("  ✅ Parsed as Comment activity");
                            ...
                            other => {
                                log!("  ⏭️ Skipping non-text message type: {:?}", other);
                            }
                        }
                    }
                    other => {
                        log!("  ⏭️ Skipping non-RoomMessage event: {:?}", other);
                    }
                }
            }
            other => {
                log!("  ⏭️ Skipping non-MessageLike event: {:?}", other);
            }
        }
    ...
    /// 加载 Space 标签库
    pub async fn load_space_tags(
        &self,
        space_id: &RoomId,
    ) -> Result<Vec<crate::kanban::state::kanban_state::SpaceTag>> {
        use matrix_sdk::ruma::api::client::state::get_state_events;
                                ...
                                if let Some(content) = json_value.get("content") {
                                    if let Some(tags_value) = content.get("tags") {
                                        match serde_json::from_value::<
                                            Vec<crate::kanban::state::kanban_state::SpaceTag>,
                                        >(
                                            tags_value.clone()
                                        ) {
                                            Ok(tags) => {
                                                log!(
                                                    "✅ Successfully loaded {} tags from space",
                                                    tags.len()
                                                );
                                                for tag in &tags {
                                                    log!(
    ...
    pub async fn save_space_tags(
        ...
        space
            .send_state_event_raw(event_type, state_key, raw_content)
            .await
    ...
    /// 迁移旧格式标签（标签名称 -> 标签 ID）
    pub async fn migrate_card_tags(
        &self,
        _card: &mut crate::kanban::state::kanban_state::KanbanCard,
        _space_id: &RoomId,
    ) -> Result<()> {
        // 简化方案：直接使用标签名称，不需要迁移
        Ok(())
...
impl MatrixKanbanAdapter {
    ...
    pub async fn move_card(
        ...
        log!(
            "🚚 move_card: Moving card {} from space {} to space {}",
            card_id,
            source_space_id,
            target_space_id
        );
        ...
        log!(
            "🚚 Step 1: Removing card from source space {}",
            source_space_id
        );
            ...
            .await
        {
            ...
            Err(e) => {
                log!(
                    "⚠️ Failed to remove m.space.child from source space: {:?}",
                    e
                );
            }
        }
        ...
        log!(
            "✅ Successfully moved card {} from space {} to space {}",
            card_id,
            source_space_id,
            target_space_id
        );
