...
use matrix_sdk::{
    ...
    ruma::{
        ...
        matrix_uri::MatrixId,
        MilliSecondsSinceUnixEpoch, OwnedEventId, OwnedMxcUri, OwnedRoomAliasId, OwnedRoomId,
        OwnedUserId, RoomOrAliasId, UserId,
    },
...
use matrix_sdk_ui::{
    ...
    timeline::{
        EventTimelineItem, LatestEventValue, RoomExt, TimelineDetails, TimelineEventItemId,
        TimelineItem,
    },
};
use robius_open::Uri;
...
use crate::{
    ...
    utils::{self, AVATAR_THUMBNAIL_FORMAT, RoomNameId, avatar_from_room_name},
...
pub enum MatrixRequest {
    ...
    /// Request to load a single card with all its data (metadata, todos, etc.)
    LoadCard {
        card_id: OwnedRoomId,
        space_id: OwnedRoomId,
    },
...
async fn matrix_worker_task(
    ...
    let mut subscribers_own_user_read_receipts: BTreeMap<OwnedRoomId, JoinHandle<()>> =
        BTreeMap::new();
        ...
        match request {
            ...
            MatrixRequest::InviteUser { room_id, user_id } => {
                let (timeline, sender) = {
                    ...
                    let Some(room_info) = all_joined_rooms.get_mut(&room_id) else {
                        error!(
                            "BUG: room info not found for invite user request {room_id}, {user_id}"
                        );
                        continue;
                    };
                    (
                ...
                let _redact_task = Handle::current().spawn(async move {
                    match timeline.redact(&timeline_event_id, reason.as_deref()).await {
                        ...
                        Err(e) => {
                            ...
                            enqueue_popup_notification(PopupItem {
                                message: format!("Failed to redact message. Error: {e}"),
                                kind: PopupKind::Error,
                                auto_dismissal_duration: None,
                            });
                        }
            ...
            MatrixRequest::LoadCard { card_id, space_id } => {
                let Some(client) = get_client() else {
                    error!("Cannot load card: Matrix client not available");
                    continue;
                };
            ...
            MatrixRequest::SaveCardTodos { card_id, todos } => {
                ...
                let _save_todos_task = Handle::current().spawn(async move {
                    ...
                    match adapter.save_card_todos(&card_id, &todos).await {
                        Ok(_) => {
                            log!("✅ Successfully saved todos for card {}", card_id);
...
static ALL_JOINED_ROOMS: Mutex<BTreeMap<OwnedRoomId, JoinedRoomDetails>> =
...
async fn start_matrix_client_login_and_sync(rt: Handle) {
    ...
    let mut space_service_task = rt.spawn(space_service_loop(
        SpaceService::new(client.clone()),
        client,
    ));
            ...
            match diff {
                VectorDiff::Append { values: new_rooms } => {
                    ...
                    if LOG_ROOM_LIST_DIFFS {
                        log!("room_list: diff Append {_num_new_rooms}");
                    }
                ...
                remove_diff @ VectorDiff::Remove {
                    ...
                    if LOG_ROOM_LIST_DIFFS {
                        log!("room_list: diff Remove at {remove_index}");
                    }
    ...
    match new_room.state {
        ...
        RoomState::Invited => {
            ...
            let latest = latest_event
                .as_ref()
                .map(|ev| get_latest_event_details(ev, &new_room.room_id));
            let room_name_id =
...
async fn update_latest_event(room: &Room) {
    ...
    let (sender_username, sender_id, timestamp, content) = match room.new_latest_event().await {
        ...
        LatestEventValue::Local {
            ...
            let Some(our_user_id) = current_user_id() else {
                return;
            };
            (
    ...
    let latest_message_text = text_preview_of_timeline_item(&content, &sender_id, &sender_username)
        .format_with(&sender_username, true);
