...
use matrix_sdk::{
    OwnedServerName, RoomDisplayName,
    media::{MediaFormat, MediaRequestParameters},
    room::RoomMember,
    ruma::{
        EventId, MatrixToUri, MatrixUri, OwnedEventId, OwnedMxcUri, OwnedRoomId, UserId,
        events::{
            receipt::Receipt,
            room::{
...
use matrix_sdk_ui::timeline::{
    ...
    MemberProfileChange, MembershipChange, MsgLikeContent, MsgLikeKind, OtherMessageLike,
    PollState, RoomMembershipChange, TimelineDetails, TimelineEventItemId, TimelineItem,
...
use crate::{
    ...
    room::{
        BasicRoomDetails, room_input_bar::RoomInputBarState, typing_notice::TypingNoticeWidgetExt,
    },
    shared::{
...
impl Widget for RoomScreen {
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        let user_profile_sliding_pane =
            self.user_profile_sliding_pane(ids!(user_profile_sliding_pane));
        ...
        if let Event::Actions(actions) = event {
            for (index, wr) in portal_list.items_with_actions(actions) {
                ...
                if wr.button(ids!(invite_user_button)).clicked(actions) {
                    ...
                    {
                        log!(
                            "invite_user_button clicked: index {index}, details: {:?}",
                            event_tl_item
                        );
                        ...
                        let content = ConfirmationModalContent {
                            ...
                            body_text: format!(
                                "Are you sure you want to invite {username} to this room?"
                            )
                            .into(),
                        ...
                        cx.action(InviteAction::ShowConfirmationModal(RefCell::new(Some(
                            content,
                        ))));
                    }
                }
            ...
            for action in actions {
                    ...
                    action.as_widget_action().cast()
                {
                    ...
                    if self
                        .room_name_id
                        .as_ref()
                        .is_some_and(|rn| rn.room_id() == &profile_and_room_id.room_id)
                    {
            ...
            */
            ...
            self.jump_to_bottom_button(ids!(jump_to_bottom))
...
impl RoomScreen {
            ...
            match update {
                ...
                TimelineUpdate::PaginationError { error, direction } => {
                    ...
                    enqueue_popup_notification(PopupItem {
                        message: utils::stringify_pagination_error(
                            &error,
                ...
                TimelineUpdate::MessageEdited {
                    ...
                    self.view
                        .room_input_bar(ids!(room_input_bar))
                ...
                TimelineUpdate::InviteSent { result, .. } => match result {
    ...
    fn handle_image_click(
        ...
        cx.action(ImageViewerAction::Show(LoadState::Loading(
            ...
            Some(ImageViewerMetaData {
                ...
                avatar_parameter: Some((tl_state.room_id.clone(), event_tl_item.clone())),
            }),
        )));
    ...
    pub fn set_displayed_room(&mut self, cx: &mut Cx, room_name_id: &RoomNameId) {
...
impl RoomScreenRef {
    ...
    pub fn set_displayed_room(&self, cx: &mut Cx, room_name_id: &RoomNameId) {
            ...
            match msg.msgtype() {
                ...
                _ => {
                    ...
                    if existed && item_drawn_status.content_drawn {
                        ...
                        item.label(ids!(content.message))
        ...
        if !is_server_notice {
            ...
            let (username, profile_drawn) =
                set_username_and_get_avatar_retval.unwrap_or_else(|| {
                    item.avatar(ids!(profile.avatar))
                        .set_avatar_and_get_username(
                            cx,
                            room_id,
                            event_tl_item.sender(),
...
impl SmallStateEventContent for RoomMembershipChange {
    fn populate_item_content(
        ...
        item.button(ids!(invite_user_button))
...
fn populate_small_state_event(
    ...
    let username = username_opt.unwrap_or_else(|| {
        ...
        if let Some(dt) = unix_time_millis_to_datetime(event_tl_item.timestamp()) {
            item.timestamp(ids!(left_container.timestamp))
                .set_date_time(cx, dt);
        }
        new_drawn_status.profile_drawn = profile_drawn;
        username
